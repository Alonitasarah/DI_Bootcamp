...
function planeRideCost(destination){
    if(destination === "Londre") return 183
...
function totalVacationCost() {
    ...
    while (!isOnlyNumbers(hotelAnswer)) {
        hotelAnswer  = prompt("Combien de nuits aimeriez-vous rester?");
    }  
    while (!isOnlyNumbers(carAnswer)) {
         carAnswer = prompt("Combien de jours souhaitez-vous louer la voiture?")
    }
    while (destination == "" || includesNumbers(destination)) {
        destination = prompt("où allez-vous ?");
    }
